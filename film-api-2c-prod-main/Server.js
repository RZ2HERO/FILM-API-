...
app.post(`auth/register-admin`, authenticateToken, authorizeRole(`admin`), async (req, res, next) => {
    ...
    if(!username || !password || password.length > 6 || !role) {
        ...
        const hashedPassword = await bcrypt.hash(password, salt);    
        const sql = ` Insert into users (username, password, role) values ($1, $2, $3) Returning id, username`; 
        const result = await db.query(sql, [username.toLowerCase(), hashedPassword, role]); 
        res.status(201).json(result.rows[0]);       
    } catch (err){
        if (err.code === `23505`) {
            return res.status(409).json({error: `Username sudah digunakan`});
